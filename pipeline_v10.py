...
temizlik_skoru = float(np.mean([
    max(0.0, (1.0 - metr[k]['mae'] / (float(df[k].max() - df[k].min()) + 1e-9)) * 100)
    for k in kolonlar
]))
...
try:
    ...
    def _model_yukle_ya_da_egit(dosya, builder_fn, Xl, yl, split, etiket):
        ...
        try:
            ...
            try:
                keras_yol = dosya + ".keras"
                m.save(keras_yol)
                print(f"    {etiket} kaydedildi ✓ ({keras_yol})")
            except Exception as e_save2:
                print(f"    {etiket} kaydedilemedi ({e_save1} / {e_save2}) — bellekte devam")
        return m
...
def lstm_layout():
    ...
    # ── Grafik 1: Geçmiş tahmin + gelecek tahmin TEK grafikte ──────
    fig1 = go.Figure()

    # Geçmiş: Actual vs BiLSTM (test seti)
    n_test = len(y_gercek)
    x_gecmis = list(range(n_test))
    fig1.add_trace(go.Scatter(
        x=x_gecmis, y=y_gercek[:, 0],
        name='Actual (Test)', line=dict(color=GRN, width=1.2)
    ))
    fig1.add_trace(go.Scatter(
        x=x_gecmis, y=y_pred[:, 0],
        name='BiLSTM Forecast (Past)', line=dict(color=BLU, width=1.2, dash='dot')
    ))

    # Gelecek tahmin — geçmişin hemen sağına eklenir
    if gelecek is not None:
        x_gelecek = list(range(n_test, n_test + len(gelecek)))
        # Bağlantı noktası: geçmişin son actual değerinden başlat
        fig1.add_trace(go.Scatter(
            x=[n_test - 1] + x_gelecek,
            y=[float(y_gercek[-1, 0])] + list(gelecek),
            name=f'Future Forecast ({TAHMIN} min)',
            line=dict(color=ORG, width=2, dash='dash'),
            fill='tozeroy', fillcolor='rgba(240,136,62,0.07)'
        ))
        # Gelecek bölgesi için dikey ayraç
        fig1.add_vline(
            x=n_test - 0.5, line_dash='dash', line_color=TXT2,
            annotation_text='▶ Gelecek', annotation_font_color=TXT2,
            annotation_position='top right'
        )

    fig1.update_layout(**gs(
        f'BiLSTM Solar Wind Speed — Geçmiş Tahmin + Gelecek {TAHMIN} dk  |  MAE={mae_lstm:.4f} km/s'
    ))
    ...
    return html.Div([
        kart([html.Div("🧠 BiLSTM Model - Saved/Loaded (instant demo ready)",
              ...
              html.Div(f"Model file: {LSTM_MODEL_DOSYA}  |  Kp model: {LSTM_KP_DOSYA}  |  "
                       ...
                       f"🟠 Turuncu kesikli = gelecek {TAHMIN} dk tahmini",
                       style={'color':TXT2,'fontSize':'10px'})]),
        kart([dcc.Graph(figure=fig1, style={'height':'300px'}, config={'displayModeBar':True,'scrollZoom':True})],
...
def analiz_guncelle(sensor):
    ...
    for fidx, det in flare_detay.items():
        if det['sinif'].startswith('X'):  # sadece X sınıfı flare göster
            f1.add_vline(x=fidx, line_dash='dot', line_color=ORG, opacity=0.5,
